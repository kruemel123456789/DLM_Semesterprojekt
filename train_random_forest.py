...
def main():        
    ...
    vali_data_path0 = 'train_res/vali/0'
    vali_data_path1 = 'train_res/vali/1'
    vali_data_path2 = 'train_res/vali/2'
    vali_data_path3 = 'train_res/vali/3'
    vali_data_path4 = 'train_res/vali/4'

    vali_paths0 = glob(vali_data_path0+'/*.tiff')
    vali_paths1 = glob(vali_data_path1+'/*.tiff')
    vali_paths2 = glob(vali_data_path2+'/*.tiff')
    vali_paths3 = glob(vali_data_path3+'/*.tiff')
    vali_paths4 = glob(vali_data_path4+'/*.tiff')
    
    # image names
    vali_names0 = [im.split('/')[-1].split('.')[0] for im in vali_paths0]
    vali_labels0 = 0 * np.ones(len(vali_names0))
    vali_names1 = [im.split('/')[-1].split('.')[0] for im in vali_paths1]
    vali_labels1 = np.ones(len(vali_names1))
    vali_names2 = [im.split('/')[-1].split('.')[0] for im in vali_paths2]
    vali_labels2 = 2 * np.ones(len(vali_names2))
    vali_names3 = [im.split('/')[-1].split('.')[0] for im in vali_paths3]
    vali_labels3 = 3 * np.ones(len(vali_names3))
    vali_names4 = [im.split('/')[-1].split('.')[0] for im in vali_paths4]
    vali_labels4 = 4 * np.ones(len(vali_names4))


    vali_paths = np.hstack((vali_paths0,vali_paths1, vali_paths2, vali_paths3, vali_paths4))    
    vali_labels = np.hstack((vali_labels0,vali_labels1, vali_labels2, vali_labels3, vali_labels4))

    vali_generator1 = DataGenerator(batch_size=batch_size).generate(vali_paths)
    vali_generator2 = DataGenerator(batch_size=batch_size).generate(vali_paths)
    vali_generator3 = DataGenerator(batch_size=batch_size).generate(vali_paths)
    #Xval= []
    #for i in range(len(vali_paths)//batch_size):
    #    Xval.extend(next(vali_generator))
    ...
    # predict
    y_preds1_rf = model1.predict_generator(test_generator1,
                                      verbose=1,
                                      workers=16,
                                      use_multiprocessing=True,
                                      steps = len(image_paths)//batch_size)
    
    y_preds2_rf = model2.predict_generator(test_generator2,
                                      verbose=1,
                                      workers=16,
                                      use_multiprocessing=True,
                                      steps = len(image_paths)//batch_size)
    y_preds3_rf = model3.predict_generator(test_generator3,
                                      verbose=1,
                                      workers=16,
                                      use_multiprocessing=True,
                                      steps = len(image_paths)//batch_size)
    
    X_val_1 = model1.predict_generator(vali_generator1,
                                      verbose=1,
                                      workers=16,
                                      use_multiprocessing=True,
                                      steps = len(image_paths)//batch_size)
    X_val_2 = model2.predict_generator(vali_generator2,
                                      verbose=1,
                                      workers=16,
                                      use_multiprocessing=True,
                                      steps = len(image_paths)//batch_size)
    X_val_3 = model1.predict_generator(vali_generator3,
                                      verbose=1,
                                      workers=16,
                                      use_multiprocessing=True,
                                      steps = len(image_paths)//batch_size)
    ...
    #X =  y_preds1
    X = np.hstack((y_preds1_rf, y_preds2_rf, y_preds3_rf))
    
    X2 = np.hstack((y_preds1_rf, y_preds2_rf))
    X3 = np.hstack((y_preds2_rf, y_preds3_rf))
    X4 = np.hstack((y_preds1_rf, y_preds3_rf))
    #X = np.hstack(y_preds3)
    y = image_labels
    
    clf1 = RandomForestClassifier(n_estimators=100)
    clf1.fit(X,y)
    clf2 = RandomForestClassifier(n_estimators=100, max_depth = None, min_samples_split = 2)
    clf2.fit(X2,y)
    ...
    scores = np.mean(cross_val_score(clf2, X_val, vali_labels[0:495]))
